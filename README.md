...
| 记录想法 | 卡片之间拖出连线（可带标签）；任意卡片可加备注、编辑标签；便签卡直接承载文字想法 |
| 文件进出 | 从资源管理器拖入文件（自动判断落点）；`Ctrl+V` 粘贴剪贴板截图；图片可打开原图 |
| 安全兜底 | 「移除」只是把文件移入 `_已移除`，不真删除、可恢复；撤销 / 重做保留 50 步，含被移除文件的回滚 |
| 视图与外观 | 滚轮缩放 10% ~ 400%（以鼠标为锚点，`Ctrl+0` 复位）、画布平移；深色模式且记住偏好 |
| 数据持久 | 布局与视口随文件夹保存在 `.mindscape\layout.json`，拷走文件夹就拷走整个画布 |
...
覆盖安装**不会影响已有数据** —— 空间列表在 `%APPDATA%\Mindscape\`，布局在各自空间文件夹的 `.mindscape\` 内，都在安装目录之外，卸载重装也不会动。
...
- **大图画布的内存占用** —— 图片卡片直接加载原图（不生成缩略图，保证空间文件夹只含原文件与隐藏的布局文件）；缩小视图下多张高分辨率原图同屏时，解码内存与渲染开销随原图分辨率增长。
