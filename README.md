...
| 摆放与整理 | 卡片拖动 / 缩放 / 框选多选 / 复制粘贴，对齐辅助线与吸附；分区框支持画布右键新建（同时生成同名文件夹）、整组拖动、折叠、重命名、指定颜色、自定义宽高 |
...
| 视图与外观 | 滚轮缩放 10% ~ 400%（以鼠标为锚点，`Ctrl+0` 复位、`Ctrl+Alt+0` 缩放到全部内容）、画布平移；顶栏一键切换深色 / 浅色并记住偏好 |
| 快捷键与设置 | 设置面板为可拖动 / 缩放的弹窗，内分「快捷键」「版本更新」页；快捷键页可查看、改绑、恢复默认并自动检测冲突，绑定关系持久保存 |
...
| 单元测试 | `pnpm test` | 834 passed / 62 个测试文件 |
| Rust 单元测试 | `cargo test`（于 `src-tauri/`） | 47 passed |
| 代码检查 | `pnpm lint` | 无错误 |
| 生产构建 | `pnpm build` | 通过 |
