...
| 摆放与整理 | 卡片拖动 / 缩放 / 框选多选 / 复制粘贴，对齐辅助线与吸附；分区框支持画布右键新建（同时生成同名文件夹）、整组拖动、折叠、重命名、指定颜色、自定义宽高；图片卡可右键重命名文件（磁盘文件与卡片一起改名，可撤销） |
...
| 视图与外观 | 滚轮缩放 10% ~ 400%（以鼠标为锚点，`Ctrl+0` 复位、`Ctrl+Alt+0` 缩放到全部内容）、画布平移、小地图概览；顶栏一键切换深色 / 浅色并记住偏好 |
...
- **插件系统仍在完善** —— 设置面板「插件」页与内置色卡 / 待办插件已可用；生命周期钩子尚未在生产代码埋点，外部插件（`manifest.json` + `index.js`）的动态加载链路已通但尚未经真机完整验证。
