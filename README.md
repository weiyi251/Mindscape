...
**当前版本 `v0.9.0`（预览版），已在 GitHub 发布** —— 到 [Releases](https://github.com/weiyi251/Mindscape/releases/latest) 下载，支持 Windows 10/11 x64。应用内置自动更新，之后的版本可一键升级，无需再手动下载。

计划书范围内的功能已全部实现，画布现在能做的事：

| 能力 | 说明 |
|---|---|
| 文件夹 → 画布 | 新建空间指向一个已有文件夹，文件自动网格铺开：图片卡（直接加载原图，悬停显示文件名与分辨率）、文件卡（扩展名徽标）、便签卡（纯文字） |
| 摆放与整理 | 卡片拖动 / 缩放 / 框选多选 / 复制粘贴，对齐辅助线与吸附；**多选后可右键对齐（六向）/ 等距分布，也可一次把整批卡片移入分区；单张卡片可「锁定」位置与尺寸**（整理大画布时防误拖）；分区框支持画布右键新建（同时生成同名文件夹）、整组拖动、折叠、重命名、指定颜色、自定义宽高；图片卡可右键重命名文件（磁盘文件与卡片一起改名，可撤销） |
| 记录想法 | 卡片之间拖出连线（可带标签）；任意卡片可加备注、编辑标签；便签卡双击即可在卡内直接编辑文字，还可自选 7 色柔和便签纸颜色 |
| 待办清单 | 内置待办卡片插件：逐条输入待办、勾选完成、自动编号、拖动排序、已完成置底 / 置顶，卡片高度随内容自适应；**每条待办都有独立连线点，连线可以精确到某一条待办** |
| 文件进出 | 从资源管理器拖入文件（自动判断落点）；`Ctrl+V` 粘贴剪贴板截图；**跨应用复制粘贴**：空间内复制的文件可粘贴到资源管理器 / 桌面等任意外部软件，外部复制的文件也可 `Ctrl+V` 粘贴进空间；**跨画布复制粘贴**：复制后到另一空间直接粘贴，粘到画布之外（聊天框 / 记事本）就是一段段文字；图片可打开原图 |
| 安全兜底 | 「移除」只是把文件移入 `_已移除`，不真删除、可恢复（已移除视图可对单个文件「彻底删除」，确认后真删、不可撤销）；撤销 / 重做保留 50 步，含被移除文件的回滚 |
| 文件与画布一致 | 磁盘上的子文件夹若还没有分区框，顶部会提示并可**一键补齐**；在资源管理器等处改动过空间文件夹后，切回窗口会提示「文件夹有 N 处外部变动」并提供「重新扫描」——磁盘上消失的文件**移入「已移除」视图**，不会静默消失，也不会自动合并 |
| 视图与外观 | 滚轮缩放 10% ~ 400%（以鼠标为锚点，`Ctrl+0` 复位、`Ctrl+Alt+0` 缩放到全部内容）、画布平移、小地图概览；顶栏一键切换深色 / 浅色并记住偏好；图片按**显示尺寸分档加载**（缩小视图用缩略图、放大回原图），同屏大量大图时内存占用更低 |
| 插件扩展 | 设置面板「插件」页统一管理（查看 / 启停 / 卸载 / 重载 / 刷新）；内置**色卡插件**（自定义颜色与尺寸生成纯色 PNG，色号悬停显示）与**待办卡片插件**；画布右键菜单支持插件扩展 |
| 快捷键与设置 | 设置面板为可拖动 / 缩放的弹窗，内分「快捷键」「版本更新」「插件」页；快捷键页可查看、改绑、恢复默认并自动检测冲突，绑定关系持久保存 |
...
- **插件系统仍在完善** —— 设置面板「插件」页与内置色卡 / 待办插件已可用；七个生命周期钩子（卡片新建 / 移除 / 移动、分区改名、卡片选中、空间打开与关闭）已在生产代码埋点，外部插件的动态加载链路已通并配有官方示例（`docs/examples/example.dated-note/`），但**尚未经真机完整验证**。
- **深色模式** —— 配色变量、切换入口与全量对比度走查都已完成（数据与核对清单见 `docs/界面走查与截图清单.md`），剩下的是逐项主观观感确认。
- **大图画布的内存占用** —— 图片卡片按显示尺寸分档加载：卡片在屏幕上小于约 260px 宽时改用缩略图、放大后换回原图，缩略图缓存在 `%APPDATA%\Mindscape\cache\thumbs\`（**空间文件夹里只有你自己的文件**）。该缓存目前不自动清理，长期使用会持续累积（每张几十至几百 KB）。
...
| 层 | 选型 |
|---|---|
| 桌面框架 | Tauri v2（复用系统 WebView2，实测安装包 2.6 ~ 4.0 MB） |
