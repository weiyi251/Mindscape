...
- 全量基线（2026-09-13）：Vitest **771 passed / 60 文件**、tsc 0 错、eslint 0 error（1 条既有 warning）、cargo test **47 passed**、vite build 通过（392.25 kB / gzip 120.56 kB）
...
pnpm test           # vitest run（771 tests）
pnpm lint           # eslint
...
| 2026-09-13 | （本次） | `src/canvas/interaction/{zoomKeys.ts, zoomKeys.test.ts}`、`src/canvas/Canvas.tsx`、`src/__guards__/architecture.test.ts`、`vite.config.ts`、`CHANGELOG.md` | **「适应内容」主快捷键改 `Ctrl+Alt+0`（用户实测裁决）**。上一提交 26ce237 已按物理键位修好匹配，但用户实测仍无效——**临时键位探针**（文档捕获阶段 + window 冒泡阶段双探针，直写 `textContent` 不进 React state）证明：按下 `Ctrl+Shift+0` 时应用只收到 `Control`（ctrl=true shift=true）与 `Shift`，**数字 `0` 的 `keydown` 根本没送达**（输入法 / 系统占用了该组合键），代码层无法绕过。故 `isZoomToFitShortcut` 改为「ctrlKey && altKey」或「ctrlKey && shiftKey」——**`Ctrl+Alt+0` 为主、`Ctrl+Shift+0` 兼容**；`KeyComboLike` 增 `altKey`、`isResetViewShortcut` 补 `!altKey`；状态条按钮标签改「适应内容（Ctrl+Alt+0）」；单测 10 → **14 条**。**诊断探针已全部删除**（连带两处临时提交 `7076d8e`/`273105c`——均未推送，用 `git reset --soft f36c80b` 从本地历史移除，远端 main `f36c80b` 不受影响）；架构守卫 Canvas.tsx 阈值 1221 → **1192** 复位。另修 `vite.config.ts` 的 `server.host`：默认 `false` 时 Node 只绑 IPv6 `[::1]`，WebView2 连 `127.0.0.1` 永久 SYN_SENT → **白屏**；改 `'::'` 双栈消除随机性。门禁：**775 passed / 60 文件**、tsc 0 错、eslint 0 error（1 固有 warning）、build 392.28 kB（Rust 未动） | 用户实测反馈「Ctrl+Shift+0 还是没反应」（附探针截图） |
