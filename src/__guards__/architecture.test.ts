...
const FILE_SIZE_BUDGET: Record<string, number> = {
  ...
  // 阈值下调到 2000：锁定外抽收益，余量留给后续优化项。
  // C4 卡片锁定 + C1 多选批量移动（2026-09-20）：锁定只加 2 行 deps、批量移动
  // 改签名与透传 selectedCards，实际一度到 **2002**（超阈值 2 行）。按约定再外抽：
  // **三个右键菜单弹出入口**（取上下文 + 组装菜单 + 写浮层 state）整块搬进
  // pages/board/menuEntries.ts（6 例接线测试），Board 侧只剩一个 menuDeps useMemo，
  // 实际 **1985** 行。阈值保持 2000（余量 15），留给 C3 对齐分布 / A2 生成分区 /
  // A1 外部变动提示条。
