...
import { nextSpaceId } from '@/core/id'
import { nowIsoSeconds } from '@/core/time'
import { localStorageProvider } from '@/core/storage/LocalFolderProvider'
import { createSpacesFileGateway, sortSpacesByLastOpened } from '@/core/storage/spacesFile'
    ...
    return {
      ...
      async load() {
        ...
        try {
          ...
          set({
            spaces: sortSpacesByLastOpened(interpreted.file.spaces),
            status: 'ready',
      ...
      async createSpace(input) {
        ...
        const next = sortSpacesByLastOpened([space, ...existing])
        await persist(next)
        set({ spaces: next })
        return space
      },
      ...
      async openSpace(id) {
        ...
        const next = sortSpacesByLastOpened(
