...
export interface CardTypeDef {
   ...
   *   · 'widthHeight'（2026-09-18）—— 左缘调宽 + 下缘调高：高度仍以内容自适应
   *     为下限（实测「只增不减」），但用户可拖出更大的卡片留白。
   */
  resizeHandles?: 'default' | 'widthOnly' | 'widthHeight'
  /**
   * 该类型卡片参与 Ctrl+F 搜索的自有文本（2026-09-20 搜索打通插件卡）。
   * 返回「标题 / 条目正文」等用户输入的内容，宿主把它作为一个独立字段
   * （标签「内容」）参与子串匹配；缺省 = 该类型只有文件名 / 备注 / 分区名参与搜索。
   * 纯函数：不应读 DOM、不应有副作用。
   */
  searchText?: (card: Card) => string
}
