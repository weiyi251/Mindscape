...
//
// 为什么需要它：
//   把「菜单项的元数据（id / label / 顺序 / 过滤）」与「动作实现」解耦：
//     · 菜单配置写完整（真实 id、真实中文名、真实顺序），插件也能往里追加；
//     · 动作实现由登记方调用 registerAction 登记 —— 核心的 11 个动作已由
//       pages/Board.tsx 在挂载时全部登记完毕，**并非「尚未实现」**；
//     · 未登记时点击给出明确的中文 console 提示（不静默、不抛错）：
//       这条现在主要服务于**插件**（注册了菜单项但忘记登记动作），
//       以及将来「动作由某个模块按需提供」的场景，故保留。
//
